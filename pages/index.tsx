    ...
    return (
        <Grid container p={2}>
            <MovieContainer title={'ویژه'} link={'/animation'}>
                <MovieCard name={'Flash'} image={image1} size={'Horizontal'}/>
                <MovieCard name={'Batman'} image={image2} size={'Horizontal'}/>
                <MovieCard name={'Spiderman'} image={image3} size={'Horizontal'}/>
                <MovieCard name={'Batman'} image={image4} size={'Horizontal'}/>
            </MovieContainer>
            <MovieContainer title={'تازه ها'} link={'drama'}>
                {movies.slice(9,17).map( (movie:any) => <MovieCard key={movie.id} name={movie.title} image={movie.image} size={'Vertical'}/>)}
            </MovieContainer>
        </Grid>
    )
}
